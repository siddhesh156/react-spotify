...
export const fetchSongs = (accessToken) => {
  return (dispatch) => {
    ...
    const request = new Request(
      `https://api.spotify.com/v1/tracks?ids=5X6TnKT37TaSDkFm0598Uo,28ZCSdsVrt0BCaBhRI0HXq,1bhUWB0zJMIKr9yVPrkEuI`,
      {
        headers: new Headers({
...
export const searchSongs = (searchTerm, accessToken) => {
  return (dispatch) => {
    ...
    const request = new Request(
      `https://api.spotify.com/v1/search?q=${searchTerm}&type=track&limit=10&offset=5`,
      {
        headers: new Headers({
          Authorization: 'Bearer ' + accessToken,
          Accept: 'application/json',
    ...
    fetch(request)
      ...
      .then((res) => {
        ...
        console.log("search song=> ",res.items);
...
export const fetchRecentlyPlayed = (accessToken) => {
  return (dispatch) => {
    ...
    const request = new Request(
      `https://api.spotify.com/v1/me/player/recently-played?limit=10&after=1484811043508`,
      {
        headers: new Headers({
          Authorization: 'Bearer ' + accessToken,
        }),
